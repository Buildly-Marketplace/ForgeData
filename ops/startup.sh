...
# ForgeData Setup and Deployment Script
# This script handles environment setup, dependency installation, and server management
# Usage: ./ops/startup.sh [setup|start|stop|restart|health] [OPTIONS]
...
DB_PATH="${PROJECT_ROOT}/forgedata.db"
...
Commands:
    ...
    start               Start the ForgeData server
    stop                Stop the ForgeData server
    restart             Restart the ForgeData server (stop then start)
...
Examples:
    ...
    ./ops/startup.sh health                   # Check health status
...
case "$COMMAND" in
    setup)
        print_header "ForgeData Setup and Deployment"
        ...
        if [ "$(printf '%s\n' "$REQUIRED_VERSION" "$CURRENT_VERSION" | sort -V | head -n1)" != "$REQUIRED_VERSION" ]; then
            print_error "Python $REQUIRED_VERSION or higher is required (found $CURRENT_VERSION)"
            exit 1
        fi
...
"
    ...
    start)
        print_header "Starting ForgeData Server"
        ...
        echo ""
        print_info "╔════════════════════════════════════════╗"
        print_info "║     ForgeData Server Starting          ║"
        print_info "║     Port: $PORT                         ║"
        print_info "║     URL: http://localhost:$PORT        ║"
        print_info "║     API Docs: http://localhost:$PORT/api/docs ║"
    ...
    stop)
        print_header "Stopping ForgeData Server"
